...
async function migrateContactsToMessaging() {
  try {
    // Connect to MongoDB
    ...
    // Get all existing contacts with old schema
    ...
    for (const contact of oldContacts) {
      // Check if this contact already has the new schema (has messages array)
      if (contact.messages && Array.isArray(contact.messages)) {
        console.log(`Contact ${contact.email} already migrated, skipping...`);
        continue;
      }
      ...
      const message = {
        subject: oldData.subject || 'Contact Form Submission',
